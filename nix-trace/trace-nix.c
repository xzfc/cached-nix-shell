...
static void __attribute__((constructor)) init() {
  ...
  while (tmpdirend > tmpdir_real && tmpdirend[-1] == '/')
    tmpdirend--;
  int len =
      snprintf(tmp_prefix, sizeof tmp_prefix, "%.*s/nix-%" PRIu64 "-",
               (int)(tmpdirend - tmpdir_real), tmpdir_real, (uint64_t)getpid());
...
#define WRAPPER(RET, FUN, ARGS)                                                \
  ...
  __attribute__((used)) static void *_cns_interpose_##FUN[2]                   \
      __attribute__((section("__DATA,__interpose"))) = {&_cns_wrapper_##FUN,   \
                                                        &FUN};                 \
  static RET _cns_wrapper_##FUN ARGS
#define REAL(FUN) FUN
...
#define WRAPPER(RET, FUN, ARGS)                                                \
  ...
  RET FUN ARGS
#define REAL(FUN)                                                              \
  (_cns_real_##FUN == NULL ? (_cns_real_##FUN = dlsym(RTLD_NEXT, #FUN)) :      \
                             _cns_real_##FUN)
...
WRAPPER(int, unlinkat, (int dirfd, const char *path, int flags)) {
  ...
  if (memcmp(path, tmp_prefix + tmp_prefix_dirname_len + 1,
             tmp_prefix_basename_len) != 0 ||
...
static void print_log(char op, const char *path, const char *result) {
  ...
  fprintf(log_f,
          ...
          "%s%c"
          "%s%c",
          op, path[0] == '/' ? "" : pwd, path[0] == '/' ? "" : "/", path,
          (char)0, result, (char)0);
  fflush(log_f);
  UNLOCK(print_mutex);
}
...
static void hash_dir(char digest_s[static LEN * 2 + 1], DIR *dirp) {
  ...
  while ((ent = readdir(dirp))) {
    ...
    char ent_type = ent->d_type == DT_DIR ? 'd' :
                    ent->d_type == DT_LNK ? 'l' :
                    ent->d_type == DT_REG ? 'f' :
                                            'u';
...
static void convert_digest(char digest_s[static LEN * 2 + 1],
                           const uint8_t digest_b[static LEN]) {
  for (int i = 0; i < LEN; i++)
